...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    val selectDf = spark.sql("select rectangle, count(point) from joinResult group by rectangle order by rectangle").persist()

    return selectDf.coalesce(1) // YOU NEED TO CHANGE THIS PART
  }
